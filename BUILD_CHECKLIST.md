...
- [x] **Hash Verification**: Double-check in the database GUI that passwords are saved strictly as bcrypt hashes, never in plaintext.
- [x] **Secret Safety**: Check that `.env` and `.env.local` are both added to your `.gitignore` to prevent secret leakage.
- [x] **Production Keys**: Ensure all production API keys (`DATABASE_URL`, `NEXTAUTH_SECRET`, `RESEND_API_KEY`/`BREVO_API_KEY`, `UPSTASH_REDIS_REST_URL`) are set inside your hosting platform's (e.g. Vercel) dashboard.
- [x] **Error Integrity**: Test that login errors return a generic `"Invalid email or password"` rather than stating whether the email exists.
- [x] **Token Expirations**: Verify code matches `15 * 60 * 1000` (15 mins) for verification tokens and `60 * 60 * 1000` (1 hour) for reset tokens.
- [x] **Single-Use Verification**: Attempt to reuse a verification or password reset token to confirm that the API rejects it on subsequent tries.
- [x] **Middleware Integrity**: Navigate to `/dashboard` directly in an incognito page to guarantee a silent, automatic redirect to `/login` occurs.
